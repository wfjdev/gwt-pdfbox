...
public class PDCIDFontType2 extends PDCIDFont
{
            ...
            if (stream != null)
            {
                try
                {
                    ...
                    if (otf.isPostScript())
                    {
                        ...
                        fontIsDamaged = true;
                        DomGlobal.console.warn("Found CFF/OTF but expected embedded TTF font " + fd.getFontName());
                    }
                }
                catch (IOException e)
                {
                    fontIsDamaged = true;
                    DomGlobal.console.warn("Could not read embedded OTF for font " + getBaseFont(), e);
                }
            }
    ...
    private TrueTypeFont findFontOrSubstitute() throws IOException
    {
        ...
        {
            DomGlobal.console.warn("Using fallback font " + ttfFont.getName() + " for CID-keyed TrueType font " + getBaseFont());
        }
        return ttfFont;
    }
    ...
    public int codeToGID(int code) throws IOException
    {
        if (!isEmbedded)
        {
            ...
            if (cid2gid != null && !isDamaged)
            {
                ...
                DomGlobal.console.warn("Using non-embedded GIDs in font " + getName());
            ...
            else
            {
                ...
                if (unicode == null)
                {
                    if (!noMapping.contains(code))
                    {
                        ...
                        noMapping.add(code);
                        DomGlobal.console.warn("Failed to find a character mapping for " + code + " in " + getName());
                    }
                ...
                {
                    DomGlobal.console.warn("Trying to map multi-byte character using 'cmap', result will be poor");
                }

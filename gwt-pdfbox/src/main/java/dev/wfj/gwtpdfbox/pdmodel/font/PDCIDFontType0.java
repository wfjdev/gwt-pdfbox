...
import dev.wfj.gwtpdfbox.fontbox.cff.CFFCIDFont;
...
public class PDCIDFontType0 extends PDCIDFont
{
    ...
    public PDCIDFontType0(COSDictionary fontDictionary, PDType0Font parent) throws IOException
    {
            ...
            if (ff3Stream != null)
            {
                try (RandomAccessRead randomAccessRead = ff3Stream.getCOSObject().createView())
                {
                    if (randomAccessRead.length() > 0 && randomAccessRead.peek() == '%')
                    {
                        ...
                        DomGlobal.console.warn("Found PFB but expected embedded CFF font " + fd.getFontName());
                        fontIsDamaged = true;
                    }
                    else
                    {
                ...
                {
                    DomGlobal.console.error("Can't read the embedded CFF font " + fd.getFontName(), e);
                    fontIsDamaged = true;
                }
            }
        }
        ...
        else
        {
            ...
            {
                DomGlobal.console.warn("Using fallback " + font.getName() + " for CID-keyed font " +
                         getBaseFont());
            }
            isEmbedded = false;
    ...
    public final Matrix getFontMatrix()
    {
        if (fontMatrix == null)
        {
            ...
            else
            {
                ...
                {
                    DomGlobal.console.debug("Couldn't get font matrix - returning default value", e);
    ...
    private BoundingBox generateBoundingBox()
    {
        ...
        {
            DomGlobal.console.debug("Couldn't get font bounding box - returning default value", e);

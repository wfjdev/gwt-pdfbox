...
public class PDPageTree implements COSObjectable, Iterable<PDPage>
{
    ...
    private List<COSDictionary> getKids(COSDictionary node)
    {
        ...
        for (int i = 0, size = kids.size(); i < size; i++)
        {
            ...
            {
                DomGlobal.console.warn("COSDictionary expected, but got " +  (base == null ? "null" : base.getClass().getSimpleName()));
            }
        }
    ...
    private final class PageIterator implements Iterator<PDPage>
    {
        ...
        private void enqueueKids(COSDictionary node)
        {
            if (isPageTreeNode(node))
            {
                ...
                for (COSDictionary kid : kids)
                {
                    if (set.contains(kid))
                    {
                        ...
                        DomGlobal.console.error("This page tree node has already been visited");
                        continue;
                    }
            ...
            else
            {
                ...
                {
                    DomGlobal.console.error("Page skipped due to an invalid or missing type " + node.getCOSName(COSName.TYPE));
                }
            }
        }

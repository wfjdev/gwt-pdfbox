...
import dev.wfj.gwtpdfbox.pdmodel.font.encoding.ZapfDingbatsEncoding;
...
public class PDType1Font extends PDSimpleFont implements PDVectorFont
{
    ...
    public PDType1Font(FontName baseFont, boolean load)
    {
        ...
        if (mapping.isFallback())
        {
            ...
            {
                DomGlobal.console.debug("Couldn't get font name - setting to '?'", e);
                fontName = "?";
            }
    ...
    public PDType1Font(COSDictionary fontDictionary) throws IOException
    {
        ...
        if (fd != null)
        {
            ...
            if (fontFile3 != null)
            {
                DomGlobal.console.warn("/FontFile3 for Type1 font not supported");
            }
            ...
            if (fontFile != null)
            {
                ...
                {
                    DomGlobal.console.warn("Can't read damaged embedded Type1 font " + fd.getFontName(), e);
                    fontIsDamaged = true;
                }
                catch (IOException e)
                {
                    DomGlobal.console.error("Can't read the embedded Type1 font " + fd.getFontName(), e);
                    fontIsDamaged = true;
                }
            }
        }
        ...
        else
        {
            ...
            if (mapping.isFallback())
            {
                DomGlobal.console.warn("Using fallback font " + genericFont.getName() + " for " + getBaseFont());
            }
        }
        readEncoding();
    ...
    private int repairLength1(byte[] bytes, int length1)
    {
        ...
        if (length1 - offset != 0 && offset > 0)
        {
            ...
            //{
                DomGlobal.console.warn("Ignored invalid Length1 " + length1 + " for Type 1 font " + getName());
            //}
            return offset;
        }
    ...
    private int repairLength2(byte[] bytes, int length1, int length2)
    {
        ...
        {
            DomGlobal.console.warn("Ignored invalid Length2 " + length2 + " for Type 1 font " + getName());
    ...
    public final Matrix getFontMatrix()
    {
        if (fontMatrix == null)
        {
            ...
            {
                DomGlobal.console.debug("Couldn't get font matrix box - returning default value", e);

...
import dev.wfj.gwtpdfbox.cos.ICOSParser;
...
public class COSParser extends BaseParser implements ICOSParser
{
        ...
        while (prev > 0)
        {
            ...
            if (source.peek() == X)
            {
                ...
                if(trailer.containsKey(COSName.XREF_STM))
                {
                    ...
                    {
                        DomGlobal.console.warn("/XRefStm offset " + streamOffset + " is incorrect, corrected to " + fixedOffset);
                    ...
                    if (streamOffset > 0)
                    {
                        ...
                        catch (IOException ex)
                        {
                            if (isLenient)
                            {
                                DomGlobal.console.error("Failed to parse /XRefStm at offset " + streamOffset, ex);
                            }
                            else
                            {
                                throw ex;
                    ...
                    else
                    {
                        if(isLenient)
                        {
                            DomGlobal.console.error("Skipped XRef stream due to a corrupt offset:"+streamOffset);
                        }
                        else
                        {
    ...
    private long getStartxrefOffset() throws IOException
    {
        ...
        if (bufOff < 0)
        {
            if (isLenient) 
            {
                ...
                bufOff = buf.length;
                DomGlobal.console.debug("Missing end of file marker '" + new String(EOF_MARKER) + "'");
            } 
            else 
            {
            ...
            throws IOException
    {
        ...
        if (offsetOrObjstmObNr == null && isLenient)
        {
            ...
            if (offsetOrObjstmObNr != null)
            {
                DomGlobal.console.debug("Set missing offset " + offsetOrObjstmObNr + " for object " + objKey);
            ...
            throws IOException
    {
        ...
        if (!endObjectKey.startsWith(ENDOBJ_STRING))
        {
            if (isLenient)
            {
                DomGlobal.console.warn("Object (" + readObjNr + ":" + readObjGen + ") at offset "
    ...
    protected COSBase parseObjectStreamObject(long objstmObjNr, COSObjectKey key) throws IOException
    {
        ...
        if (objstmBaseObj instanceof COSStream)
        {
            ...
            catch (IOException ex)
            {
                if (isLenient)
                {
                    DomGlobal.console.error("object stream " + objstmObjNr
    ...
    private COSNumber getLength(final COSBase lengthBaseObj) throws IOException
    {
        ...
        if (lengthBaseObj instanceof COSObject)
        {
            ...
            {
                DomGlobal.console.warn("Length object (" + lengthObj.getObjectNumber() + " "
    ...
    protected COSStream parseCOSStream(COSDictionary dic) throws IOException
    {
        ...
        {
            DomGlobal.console.warn("stream ends with 'endobj' instead of 'endstream' at offset "
                    + source.getPosition());
        ...
        {
            DomGlobal.console.warn("stream ends with '" + endStream + "' instead of 'endstream' at offset "
                    + source.getPosition());
    ...
    private boolean validateStreamLength(long streamLength) throws IOException
    {
        ...
        if (expectedEndOfStream > fileLen)
        {
            ...
            DomGlobal.console.warn("The end of the stream is out of range, using workaround to read the stream, "
        ...
        else
        {
            ...
            {
                streamLengthIsValid = false;
                DomGlobal.console.warn("The end of the stream doesn't point to the correct offset, using workaround to read the stream, "
                        + "stream start position: " + originOffset + ", length: " + streamLength
                        + ", expected end position: " + expectedEndOfStream);
            }
        ...
        if (isWhitespace(nextValue))
        {
            ...
            if (isDigit())
            {
                ...
                catch (IOException exception)
                {
                    ...
                    DomGlobal.console.debug("No Xref stream at given location " + startXRefOffset, exception);
                    source.seek(startXRefOffset);
                }
            }
        }
    ...
    private long calculateXRefFixedOffset(long objectOffset) throws IOException
    {
        if (objectOffset < 0)
        {
            DomGlobal.console.error("Invalid object offset " + objectOffset + " when searching for a xref table/stream");
            return 0;
        }
        ...
        if (newOffset > -1)
        {
            DomGlobal.console.debug("Fixed reference for xref table/stream " + objectOffset + " -> " + newOffset);
            return newOffset;
        }
        DomGlobal.console.error("Can't find the object xref table/stream at offset " + objectOffset);
        return 0;
    }
    ...
    private boolean validateXrefOffsets(Map<COSObjectKey, Long> xrefOffset) throws IOException
    {
        ...
        for (Entry<COSObjectKey, Long> objectEntry : xrefOffset.entrySet())
        {
            ...
            if (objectOffset != null && objectOffset >= 0)
            {
                ...
                {
                    DomGlobal.console.debug("Stop checking xref offsets as at least one (" + objectKey
    ...
    private void checkXrefOffsets() throws IOException
    {
        ...
        if (!validateXrefOffsets(xrefOffset))
        {
            ...
            {
                DomGlobal.console.debug("Replaced read xref table with the results of a brute force search");
                xrefOffset.clear();
            ...
            Map<COSObjectKey, Long> xrefOffset) throws IOException
    {
        ...
        try 
        {
            ...
            if (source.getPosition() == offset)
            {
                ...
                if (source.getPosition() < offset)
	            {
	                ...
	                else
	                {
	                    ...
	                    {
	                        DomGlobal.console.debug("Found the object " + newObjKey + " instead of " //
            ...
            {
                DomGlobal.console.warn("found wrong object number. expected [" + objectKey.getNumber() + "] found [" + foundObjectNumber + "]");
                if (!isLenient)
                {
                    return null;
                }
        ...
        catch (IOException exception)
        {
            ...
            DomGlobal.console.debug("No valid object at given location " + offset + " - ignoring", exception);
        }
        return null;
    }
    ...
    private int checkPagesDictionary(COSDictionary pagesDict, Set<COSObject> set)
    {
        ...
        if (kidsArray != null)
        {
            ...
            for (COSBase kid : kidsList)
            {
                ...
                {
                    DomGlobal.console.warn("Removed null object " + kid + " from pages dictionary");
                    kidsArray.remove(kid);
                }
        ...
        if (isLenient)
        {
            ...
            while (nextCharacter != 't' && isDigit(nextCharacter))
            {
                if (source.getPosition() == trailerOffset)
                {
                    ...
                    DomGlobal.console.warn("Expected trailer object at offset " + trailerOffset
                            + ", keep trying");
                }
                readLine();
    ...
    private boolean parseHeader(String headerMarker, String defaultVersion) throws IOException
    {
        ...
        if (header.startsWith(headerMarker) && !header.matches(headerMarker + "\\d.\\d"))
        {
            if (header.length() < headerMarker.length() + 3)
            {
                ...
                DomGlobal.console.debug("No version found, set to " + defaultVersion + " as default.");
            }
            else
            {
        ...
        {
            DomGlobal.console.debug("Can't parse the header version.", exception);
        }
        if (headerVersion < 0)
        {
            if (isLenient)
    ...
    protected boolean parseXrefTable(long startByteOffset) throws IOException
    {
        ...
        {
            DomGlobal.console.warn("skipping empty xref table");
            return false;
        }
        ...
        while(true)
        {
            ...
            {
                DomGlobal.console.warn("Unexpected XRefTable Entry: " + currentLine);
                return false;
            }
            // first obj id
            long currObjID;
            ...
            {
                DomGlobal.console.warn("XRefTable: invalid ID for the first object: " + currentLine);
                return false;
            }
            ...
            {
                DomGlobal.console.warn("XRefTable: invalid number of objects: " + currentLine);
                return false;
            }
            ...
            for(int i = 0; i < count; i++)
            {
                ...
                {
                    DomGlobal.console.warn("invalid xref line: " + currentLine);
                    break;
                }

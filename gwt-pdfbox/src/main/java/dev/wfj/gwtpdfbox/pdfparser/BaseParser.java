...
import dev.wfj.gwtpdfbox.cos.COSDocument;
...
public abstract class BaseParser
{
    ...
    private COSBase parseCOSDictionaryValue() throws IOException
    {
        ...
        {
            DomGlobal.console.error("expected number, actual=" + value + " at offset " + numOffset);
            return COSNull.NULL;
        }
        if (!(generationNumber instanceof COSInteger))
        {
            DomGlobal.console.error("expected number, actual=" + generationNumber + " at offset " + genOffset);
            return COSNull.NULL;
        }
        long objNumber = ((COSInteger) value).longValue();
        if (objNumber <= 0)
        {
            DomGlobal.console.warn("invalid object number value =" + objNumber + " at offset " + numOffset);
            return COSNull.NULL;
        }
        int genNumber = ((COSInteger) generationNumber).intValue();
        if (genNumber < 0)
        {
            DomGlobal.console.error("invalid generation number value =" + genNumber + " at offset " + numOffset);
            return COSNull.NULL;
        }
    ...
    protected COSDictionary parseCOSDictionary(boolean isDirect) throws IOException
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                DomGlobal.console.warn("Invalid dictionary, found: '" + c + "' but expected: '/' at offset "
                        + source.getPosition());
    ...
    private boolean parseCOSDictionaryNameValuePair(COSDictionary obj) throws IOException
    {
        ...
        {
            DomGlobal.console.warn("Empty COSName at offset " + source.getPosition());
        }
        COSBase value = parseCOSDictionaryValue();
        skipSpaces();
        if (value == null)
        {
            DomGlobal.console.warn("Bad dictionary declaration at offset " + source.getPosition());
            return false;
        }
        else if (value instanceof COSInteger && !((COSInteger) value).isValid())
        {
            DomGlobal.console.warn("Skipped out of range number value at offset " + source.getPosition());
        }
        else
        {
    ...
    protected COSArray parseCOSArray() throws IOException
    {
        ...
        while (((i = source.peek()) > 0) && ((char) i != ']'))
        {
            ...
            if( pbo instanceof COSObject )
            {
                ...
                if (po.size() > 0 && po.get(po.size() - 1) instanceof COSInteger)
                {
                    ...
                    if (po.size() > 0 && po.get(po.size() - 1) instanceof COSInteger)
                    {
                        ...
                        {
                            DomGlobal.console.warn("Invalid value(s) for an object key " + number.longValue()
                                    + " " + genNumber.intValue());
                            pbo = null;
                        }
                    }
            ...
            else
            {
                ...
                DomGlobal.console.warn("Corrupt array element at offset " + source.getPosition()
        ...
        while (c != -1)
        {
            ...
            if (ch == '#')
            {
                ...
                else
                {
                    ...
                    if (ch2 == -1 || ch1 == -1)
                    {
                        DomGlobal.console.error("Premature EOF in BaseParser#parseCOSName");
                        c = -1;
                        break;
                    }
                    source.rewind(1);
    ...
    private boolean isValidUTF8(byte[] input)
    {
        ...
        {
            DomGlobal.console.debug("Character could not be decoded using StandardCharsets.UTF_8 - returning false", e);
            return false;
        }*/
    }
    ...
    protected COSBase parseDirObject() throws IOException
    {
        ...
        default:
            ...
            {
                DomGlobal.console.warn("Skipped unexpected dir object = '" + badString + "' at offset "

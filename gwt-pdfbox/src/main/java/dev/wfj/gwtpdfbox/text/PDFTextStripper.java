...
import dev.wfj.gwtpdfbox.pdmodel.PDPageTree;
import dev.wfj.gwtpdfbox.pdmodel.common.PDRectangle;
import dev.wfj.gwtpdfbox.pdmodel.interactive.documentnavigation.outline.PDOutlineItem;
import dev.wfj.gwtpdfbox.pdmodel.interactive.pagenavigation.PDThreadBead;
import dev.wfj.gwtpdfbox.util.IterativeMergeSort;
import elemental2.dom.DomGlobal;

/**
 * This class will take a pdf document and strip out all of the text and ignore
 * the formatting and such. Please note; it
 * is up to clients of this class to verify that a specific user has the correct
 * permissions to extract text from the
 * PDF document.
 * 
 * The basic flow of this process is that we get a document and use a series of
 * processXXX() functions that work on
 * smaller and smaller chunks of the page. Eventually, we fully process each
...
public class PDFTextStripper extends LegacyPDFStreamEngine {
     ...
     * DomGlobal.console.debug("Couldn't read system properties - using defaults",
     * e);
     * }
     ...
     * example a PDF that has two columns like
     * a newspaper, we want to extract the first column and then the second column.
     * In this example the PDF would have 2
     * beads(or articles), one for each column. The size of the charactersByArticle
     * would be 5, because not all text on
     * the screen will fall into one of the articles. The five divisions are shown
     * below
     *
     ...
     * be rendered in a certain order. The order is the one that was determined by
     * the software that created the
     ...
     */
    public void writeText(PDDocument doc, StringBuilder outputStream) throws IOException {
        resetEngine();
        document = doc;
        output = outputStream;
    ...
    /**
     * This method is available for subclasses of this class. It will be called
     * after processing of the document
     * finishes.
     *
     * @param document The PDF document that is being processed.
     * @throws IOException If an IO error occurs.
     ...
     * (also referred to as a bead). This
     * assumes that the primary direction of text is left to right. Default
     * implementation is to do nothing. Subclasses
    ...
    /**
     * Start a new article, which is typically defined as a column on a single page
     * (also referred to as a bead).
     * Default implementation is to do nothing. Subclasses may provide additional
     * information.
     *
     ...
     * text, where newlines and word spacings should be placed. The text will be
     * sorted only if that feature was
     * enabled.
     *
            ...
            while (textIter.hasNext()) {
                ...
                if (lastPosition != null &&
                        (position.getFont() != lastPosition.getTextPosition().getFont() ||
                                Float.compare(position.getFontSize(),
                                        lastPosition.getTextPosition().getFontSize()) != 0)) {
                ...
                //DomGlobal.console.log("position: "+positionX+", "+positionY+", "+positionWidth+", "+positionHeight);
    ...
    protected void writeLineSeparator() throws IOException {
        output.append(getLineSeparator());
    }
    ...
    protected void writeWordSeparator() throws IOException {
        output.append(getWordSeparator());
    }
     ...
     * characters on a page. It takes care of
     * overlapping text.
     *
    ...
    protected void processTextPosition(TextPosition text) {
        ...
        if (suppressDuplicateOverlappingText) {
            ...
            //DomGlobal.console.log("processTextPosition");
            //DomGlobal.console.log(textX+", "+textY+": "+textCharacter);
     ...
     * page 1. For example in a 5 page PDF
     * document, if the start page is 1 then all pages will be extracted. If the
     * start page is 4 then pages 4 and 5 will
     ...
     * example if a 5 page PDF an endPage
     * value of 5 would extract the entire document, an end page of 2 would extract
     * pages 1 and 2. This defaults to
     ...
     * property is used if the line separator
     ...
     * algorithm will output a space
     * character if there is enough space between two words. By default a space
     * character is used. If you need and
     * accurate count of characters that are found in a PDF document then you might
     * want to set the word separator to
     * the empty string.
     *
     ...
     * only be a single article. This
     * returns a List that contains List objects, the inner lists will contain
     ...
     * other. Word paints the same
     * character several times in order to make it look bold. By setting this to
     * false all text will be extracted, which
     ...
     * appear visually on the screen. For
     * example, a PDF writer may write out all text by font, so all bold or larger
     * text, then make a second pass and
     ...
     * default PDFBox does <b>not</b> sort
     ...
     * added. Note that the default value for this has been determined from trial
     * and error.
     * 
     ...
     * spaces in text should be added. Note
     * that the default value for this has been determined from trial and error.
     * Setting this value larger will reduce
     ...
     * estimate where spaces in text should
     * be added. Note that the default value for this has been determined from trial
     * and error.
     * 
     * @return The current tolerance / scaling factor
     */
     ...
     * which the current line start can be
     * indented from the previous line start beyond which the current line start is
     ...
     * characters beyond which the current line
     ...
     * characters beyond which the current
     ...
     *                           whitespace between lines in the same
     *                           paragraph.
     */
     ...
     * position and the last text position that
     * followed a line separator to decide if the gap represents a paragraph
     * separation. This should <i>only</i> be
     ...
     * if the current vertical position
     * has dropped below the last text vertical position by at least 2.5 times the
     * current text height OR if the current
    ...
    protected void writeParagraphStart() throws IOException {
        ...
        output.append(getParagraphStart());
        inParagraph = true;
    }
    ...
    protected void writePageEnd() throws IOException {
        output.append(getPageEnd());
    }

    /**
     * returns the list item Pattern object that matches the text at the specified
     * PositionWrapper or null if the text
     * does not match such a pattern. The list of Patterns tested against is given
     * by the {@link #getListItemPatterns()}
     * method. To add to the list, simply override that method (if sub-classing) or
     * explicitly supply your own list
     ...
     * i.e. bullets, numbers, letters, Roman
     ...
     * implementation stands and falls with the given
     * word. If the word is a full line, the results will be the best. If the word
     * contains of single words or
     * characters, the order of the characters in a word or words in a line may
     * wrong, due to RTL and LTR marks and
     * characters!
     * 
     * Based on
     ...
     * {
     * try {
     * parseBidiFile(FontResources.INSTANCE.bidiMirroring().getText());
     * } catch (IOException e) {
     ...
     */
    /*
     * private static void parseBidiFile(String input) throws IOException
     * {
        ...
        for (; q < strLength; q++) {
            ...
            if (0xFB00 <= c && c <= 0xFDFF || 0xFE70 <= c && c <= 0xFEFF) {
                ...
                if (c == 0xFDF2 && q > 0
                    ...
                    builder.append(word.substring(q, q + 1).trim());
                    // builder.append(Normalizer.normalize(word.substring(q, q + 1),
     ...
     * Note that the number of entries in that
     * list may differ from the number of characters in the string due to
     * normalization.
     *
     * @author Axel Dörfler
     */
     ...
     * complete access to its state fields
     * to subclasses. Also, conceptually TextPosition is immutable while these flags
     * need to be set post-creation so it

...
import dev.wfj.gwtpdfbox.pdfparser.PDFStreamParser;
...
import dev.wfj.gwtpdfbox.pdmodel.font.PDType1Font;
import dev.wfj.gwtpdfbox.pdmodel.font.PDType3CharProc;
...
import dev.wfj.gwtpdfbox.pdmodel.graphics.pattern.PDTilingPattern;
import dev.wfj.gwtpdfbox.pdmodel.graphics.state.PDGraphicsState;
import dev.wfj.gwtpdfbox.pdmodel.graphics.state.PDTextState;
...
public abstract class PDFStreamEngine
{
    ...
    public void showTextStrings(COSArray array) throws IOException
    {
        ...
        for (COSBase obj : array)
        {
            ...
            {
                DomGlobal.console.error("Nested arrays are not allowed in an array for TJ operation: " + obj);
            }
            else
            {
                DomGlobal.console.error("Unknown type " + obj.getClass().getSimpleName() +
            ...
            throws IOException
    {
        ...
        {
            DomGlobal.console.error(exception.getMessage());
        }
        else if (exception instanceof EmptyGraphicsStackException)
        {
            DomGlobal.console.warn(exception.getMessage());
        }
    ...
    /* public void setLineDashPattern(COSArray array, int phase)
    {
        if (phase < 0)
        {
            DomGlobal.console.warn("Dash phase has negative value " + phase + ", set to 0");
            phase = 0;
        }
    ...
    public void decreaseLevel()
    {
        ...
        if (level < 0)
        {
            DomGlobal.console.error("level is " + level);
        }
    }
}

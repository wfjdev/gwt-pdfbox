...
package dev.wfj.gwtpdfbox.fontbox.ttf;
...
import dev.wfj.gwtpdfbox.fontbox.ttf.gsub.GlyphSubstitutionDataExtractor;
import dev.wfj.gwtpdfbox.fontbox.ttf.model.GsubData;
import dev.wfj.gwtpdfbox.fontbox.ttf.table.common.CoverageTable;
import dev.wfj.gwtpdfbox.fontbox.ttf.table.common.CoverageTableFormat1;
import dev.wfj.gwtpdfbox.fontbox.ttf.table.common.CoverageTableFormat2;
...
import dev.wfj.gwtpdfbox.fontbox.ttf.table.common.FeatureTable;
import dev.wfj.gwtpdfbox.fontbox.ttf.table.common.LangSysRecord;
...
import dev.wfj.gwtpdfbox.fontbox.ttf.table.common.RangeRecord;
import dev.wfj.gwtpdfbox.fontbox.ttf.table.common.ScriptRecord;
...
public class GlyphSubstitutionTable extends TTFTable
{
    ...
    private ScriptTable readScriptTable(TTFDataStream data, long offset) throws IOException
    {
        ...
        for (int i = 0; i < langSysCount; i++)
        {
            ...
            if (i > 0 && langSysTags[i].compareTo(langSysTags[i-1]) <= 0)
            {
                ...
                DomGlobal.console.error("LangSysRecords not alphabetically sorted by LangSys tag: " +
        ...
        for (int i = 0; i < featureCount; i++)
        {
            ...
            if (i > 0 && featureTags[i].compareTo(featureTags[i-1]) < 0)
            {
                ...
                if (featureTags[i].matches("\\w{4}") && featureTags[i-1].matches("\\w{4}"))
                {
                    ...
                    DomGlobal.console.debug("FeatureRecord array not alphabetically sorted by FeatureTag: " +
                              featureTags[i] + " < " + featureTags[i - 1]);
                }
                else
                {
                    DomGlobal.console.warn("FeatureRecord array not alphabetically sorted by FeatureTag: " +
                              featureTags[i] + " < " + featureTags[i - 1]);
    ...
    private LookupTable readLookupTable(TTFDataStream data, long offset) throws IOException
    {
        ...
        default:
            ...
            DomGlobal.console.debug("Type " + lookupType
            ...
            int coverageGlyphId) throws IOException
    {
        ...
        DomGlobal.console.debug("ligatureCount=" + ligatureCount);
    ...
    private int applyFeature(FeatureRecord featureRecord, int gid)
    {
        ...
        for (int lookupListIndex : featureRecord.getFeatureTable().getLookupListIndices())
        {
            ...
            {
                DomGlobal.console.debug("Skipping GSUB feature '" + featureRecord.getFeatureTag()
    ...
    public int getUnsubstitution(int sgid)
    {
        ...
        if (gid == null)
        {
            DomGlobal.console.warn("Trying to un-substitute a never-before-seen gid: " + sgid);
            return sgid;
        }
        return gid;
    }

 ...
 */
package dev.wfj.gwtpdfbox.fontbox.cff;
...
import dev.wfj.gwtpdfbox.fontbox.cff.CharStringCommand.Type1KeyWord;
import dev.wfj.gwtpdfbox.fontbox.encoding.StandardEncoding;
import dev.wfj.gwtpdfbox.fontbox.type1.Type1CharStringReader;
...
public class Type1CharString {
    ...
    private List<Number> handleType1Command(List<Number> numbers, CharStringCommand command) {
        ...
        if (type1KeyWord == null) {
            ...
            DomGlobal.console.warn("Unknown charstring command in glyph " + glyphName + " of font " + fontName);
        ...
        switch (type1KeyWord) {
            ...
            case CALLSUBR:
                // indicates an invalid charstring
                DomGlobal.console.warn("Unexpected charstring command: " + type1KeyWord + " in glyph " + glyphName + " of font "
                        + fontName);
                break;
            default:
    ...
    private void callothersubr(int num) {
        if (num == 0) {
            ...
            if (flexPoints.size() < 7) {
                DomGlobal.console.warn("flex without moveTo in font " + fontName + ", glyph " + glyphName +
            ...
            DomGlobal.console.warn("Invalid callothersubr parameter: " + num);
        }
    }
    ...
    private void rlineTo(Number dx, Number dy) {
        ...
        if (path.getCurrentPoint() == null) {
            DomGlobal.console.warn("rlineTo without initial moveTo in font " + fontName + ", glyph " + glyphName);
            path.moveTo(x, y);
        } else {
            path.lineTo(x, y);
        }
    ...
    private void rrcurveTo(Number dx1, Number dy1, Number dx2, Number dy2,
        ...
        if (path.getCurrentPoint() == null) {
            DomGlobal.console.warn("rrcurveTo without initial moveTo in font " + fontName + ", glyph " + glyphName);
            path.moveTo(x3, y3);
        } else {
    ...
    private void closeCharString1Path() {
        if (path.getCurrentPoint() == null) {
            DomGlobal.console.warn("closepath without initial moveTo in font " + fontName + ", glyph " + glyphName);
        } else {
            path.closePath();
        }
    ...
    private void seac(Number asb, Number adx, Number ady, Number bchar, Number achar) {
        ...
        try {
            ...
            DomGlobal.console.warn("invalid seac character in glyph " + glyphName + " of font " + fontName, e);
        }
        // accent character
        ...
        try {
            ...
            if (path == accentPath) {
                ...
                DomGlobal.console.warn("Path for " + baseName + " and for accent " + accentName + " are same, ignored");
                return;
            }

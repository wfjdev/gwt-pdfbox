 ...
 */
package dev.wfj.gwtpdfbox.fontbox.afm;
 ...
 *
 * @see <A href="http://partners.adobe.com/asn/developer/type/">AFM
 *      Documentation</A>
 *
...
public class AFMParser {
     ...
     */
      ...
      List<String> split = Arrays.asList(partData.split(" ;"));
      Iterator<String> splitIt = split.iterator();
      //StringTokenizer tokenizer = new StringTokenizer( partData, " ;" );
      
      
      String cc = splitIt.next();
      if( !cc.equals( CC ) )
      {
      throw new IOException( "Expected '" + CC + "' actual='" + cc + "'" );
      }
      String name = splitIt.next();
      Composite composite = new Composite(name);
      
      int partCount = parseInt(splitIt.next());
      for( int i=0; i<partCount; i++ )
      {
      String pcc = splitIt.next();
      if( !pcc.equals( PCC ) )
      {
      throw new IOException( "Expected '" + PCC + "' actual='" + pcc + "'" );
      }
      String partName = splitIt.next();
      int x = parseInt(splitIt.next());
      int y = parseInt(splitIt.next());
    ...
    private CharMetric parseCharMetric() throws IOException {
        ...
        List<String> split = Arrays.asList(metrics.split(" |\t|\n|\r|\f"));
        Iterator<String> splitIt = split.iterator();

        while (splitIt.hasNext()) {
            String nextCommand = splitIt.next();

            switch (nextCommand) {
                case CHARMETRICS_C:
                    String charCodeC = splitIt.next();
                    charMetric.setCharacterCode(parseInt(charCodeC));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_CH:
                    // Is the hex string <FF> or FF, the spec is a little
                    // unclear, wait and see if it breaks anything.
                    String charCodeCH = splitIt.next();
                    charMetric.setCharacterCode(parseInt(charCodeCH, BITS_IN_HEX));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_WX:
                    charMetric.setWx(parseFloat(splitIt.next()));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_W0X:
                    charMetric.setW0x(parseFloat(splitIt.next()));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_W1X:
                    charMetric.setW1x(parseFloat(splitIt.next()));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_WY:
                    charMetric.setWy(parseFloat(splitIt.next()));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_W0Y:
                    charMetric.setW0y(parseFloat(splitIt.next()));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_W1Y:
                    charMetric.setW1y(parseFloat(splitIt.next()));
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_W:
                    float[] w = new float[2];
                    w[0] = parseFloat(splitIt.next());
                    w[1] = parseFloat(splitIt.next());
                    charMetric.setW(w);
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_W0:
                    float[] w0 = new float[2];
                    w0[0] = parseFloat(splitIt.next());
                    w0[1] = parseFloat(splitIt.next());
                    charMetric.setW0(w0);
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_W1:
                    float[] w1 = new float[2];
                    w1[0] = parseFloat(splitIt.next());
                    w1[1] = parseFloat(splitIt.next());
                    charMetric.setW1(w1);
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_VV:
                    float[] vv = new float[2];
                    vv[0] = parseFloat(splitIt.next());
                    vv[1] = parseFloat(splitIt.next());
                    charMetric.setVv(vv);
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_N:
                    charMetric.setName(splitIt.next());
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_B:
                    BoundingBox box = new BoundingBox();
                    box.setLowerLeftX(parseFloat(splitIt.next()));
                    box.setLowerLeftY(parseFloat(splitIt.next()));
                    box.setUpperRightX(parseFloat(splitIt.next()));
                    box.setUpperRightY(parseFloat(splitIt.next()));
                    charMetric.setBoundingBox(box);
                    verifySemicolon(splitIt);
                    break;
                case CHARMETRICS_L:
                    Ligature lig = new Ligature(splitIt.next(),
                    splitIt.next());
     ...
     */
    private void verifySemicolon(Iterator<String> splitIt) throws IOException {
        if (splitIt.hasNext()) {
            String semicolon = splitIt.next();
            if (!";".equals(semicolon)) {
    ...
    /**
     * Read the next string. Throw an exception if it differs from the expected
     * command.
     * 

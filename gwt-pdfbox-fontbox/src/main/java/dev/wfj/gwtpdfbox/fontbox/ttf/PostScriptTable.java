    ...
    void read(TrueTypeFont ttf, TTFDataStream data) throws IOException
    {
        ...
        else if (Float.compare(formatType, 2.0f) == 0)
        {
            ...
            if (maxIndex >= WGL4Names.NUMBER_OF_MAC_GLYPHS)
            {
                ...
                for (int i = 0; i < nameArray.length; i++)
                {
                    ...
                    {
                        // PDFBOX-4851: EOF
                        DomGlobal.console.warn("Error reading names in PostScript table at entry " + i + " of " + 
        ...
        else if (Float.compare(formatType, 2.5f) == 0)
        {
            ...
            for (int i = 0; i < glyphNames.length; i++)
            {
                ...
                {
                    DomGlobal.console.debug("incorrect glyph name index " + index +
        ...
        else if (Float.compare(formatType, 3.0f) == 0)
        {
            ...
            DomGlobal.console.debug("No PostScript name information is provided for the font " + ttf.getName());
        }
        initialized = true;
    }

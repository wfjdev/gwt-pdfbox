...
public class CmapSubtable implements CmapLookup
{
    ...
    void processSubtype8(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        if (numGlyphs == 0)
        {
            DomGlobal.console.warn("subtable has no glyphs");
            return;
        }
        // -- Read all sub header
    ...
    void processSubtype12(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        for (long i = 0; i < nbGroups; ++i)
        {
            ...
            {
                throw new IOException("Invalid character code");
            }
            ...
            for (long j = 0; j <= endCode - firstCode; ++j)
            {
                ...
                {
                    DomGlobal.console.warn("Format 12 cmap contains an invalid glyph index");
                    break;
                }

                if (firstCode + j > 0x10FFFF)
                {
                    DomGlobal.console.warn("Format 12 cmap contains character beyond UCS-4");
                }
    ...
    void processSubtype13(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        for (long i = 0; i < nbGroups; ++i)
        {
            ...
            {
                DomGlobal.console.warn("Format 13 cmap contains an invalid glyph index");
                break;
            }
            ...
            for (long j = 0; j <= endCode - firstCode; ++j)
            {
                ...
                if (firstCode + j > 0x10FFFF)
                {
                    DomGlobal.console.warn("Format 13 cmap contains character beyond UCS-4");
                }
    ...
    void processSubtype14(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        DomGlobal.console.warn("Format 14 cmap table is not supported and will be ignored");
    }
    ...
    void processSubtype4(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        {
            DomGlobal.console.warn("cmap format 4 subtable is empty");
            return;
        }
        buildGlyphIdToCharacterCodeLookup(maxGlyphId);
    }
    ...
    void processSubtype2(TTFDataStream data, int numGlyphs) throws IOException
    {
        ...
        for (int i = 0; i <= maxSubHeaderIndex; ++i)
        {
            ...
            for (int j = 0; j < entryCount; ++j)
            {
                ...
                if (p >= numGlyphs)
                {
                    DomGlobal.console.warn("glyphId " + p + " for charcode " + charCode + " ignored, numGlyphs is " + numGlyphs);
                    continue;
                }

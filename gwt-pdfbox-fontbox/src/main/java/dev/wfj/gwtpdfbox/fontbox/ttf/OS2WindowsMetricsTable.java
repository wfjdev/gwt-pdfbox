...
public class OS2WindowsMetricsTable extends TTFTable
{
    ...
    void read(TrueTypeFont ttf, TTFDataStream data) throws IOException
    {
        ...
        {
            DomGlobal.console.debug("EOF, probably some legacy TrueType font");
            initialized = true;
            return;
        }
        if (version >= 1)
        {
            ...
            {
                version = 0;
                DomGlobal.console.warn("Could not read all expected parts of version >= 1, setting version to 0", ex);
                initialized = true;
                return;
            }
        }
        if (version >= 2)
        {
            ...
            {
                version = 1;
                DomGlobal.console.warn("Could not read all expected parts of version >= 2, setting version to 1", ex);
                initialized = true;
                return;
            }
        }
